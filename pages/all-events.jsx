  ...
  const filteredAndSortedEvents = useMemo(() => {
    ...
    if (availabilityFilter !== 'All') {
      if (availabilityFilter === 'Registration Open') {
        result = result.filter(e => e.registered < e.maxAttendees);
    ...
    // Sorting
    result.sort((a, b) => {
      if (sortBy === 'Newest First') return new Date(b.startDate) - new Date(a.startDate);
      if (sortBy === 'Oldest First') return new Date(a.startDate) - new Date(b.startDate);
      ...
      if (sortBy === 'Most Attendees') return b.maxAttendees - a.maxAttendees;
...
function EventCard({ event }) {
  ...
  return (
    ...
    >
      ...
      <div className="h-32 bg-gradient-to-br from-[#2D3340] to-[#1C202B] relative p-4 flex items-start justify-between">
        <div className="flex gap-2">
          ...
          <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border backdrop-blur-md ${event.type === 'Free' ? 'bg-[#10B981]/20 text-[#10B981] border-[#10B981]/30' : 'bg-[#F59E0B]/20 text-[#F59E0B] border-[#F59E0B]/30'}`}>
            {event.type}
          </span>
        </div>
        <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border backdrop-blur-md ${event.format === 'Online' ? 'bg-blue-500/20 text-blue-400 border-blue-500/30' : 'bg-pink-500/20 text-pink-400 border-pink-500/30'}`}>
          {event.format}
        </span>
        <ImageIcon className="absolute inset-0 m-auto w-10 h-10 text-white/5 opacity-0 group-hover:opacity-100 transition-opacity" />
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="mt-auto">
          ...
          <div className="flex justify-between items-center mt-3 pt-3 border-t border-[#2A303C]">
            <span className="text-[11px] text-[#5A6B8A]">Volunteers: {event.volunteersRequired}</span>
            {isFull ? (
